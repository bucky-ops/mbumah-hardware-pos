...
async function listDetailsHandler(
  ...
  const data = details.map((d) => ({
    ...
    // MONEY FIX (v2.3.1): serialize Prisma Decimal money fields as numbers at
    // the API boundary so the documented number contract holds at runtime.
    // Earnings
    basicSalary: Number(d.basicSalary),
    houseAllowance: Number(d.houseAllowance),
    transportAllowance: Number(d.transportAllowance),
    medicalAllowance: Number(d.medicalAllowance),
    otherAllowances: Number(d.otherAllowances),
    overtimePay: Number(d.overtimePay),
    grossPay: Number(d.grossPay),
    // Statutory deductions
    paye: Number(d.paye),
    nssf: Number(d.nssf),
    nhif: Number(d.nhif),
    housingLevy: Number(d.housingLevy),
    // Other deductions
    payeArrears: Number(d.payeArrears),
    loanDeduction: Number(d.loanDeduction),
